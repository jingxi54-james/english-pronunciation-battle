Page({
  ...
  async loadLeaderboard() {
    try {
      ...
      // 保存到本地存储
      wx.setStorageSync('leaderboard', data || []);
    } catch (error) {
      console.error('加载排行榜失败:', error);
      
      // 使用本地存储的数据
      const localData = wx.getStorageSync('leaderboard') || [];
      this.setData({
  ...
  handleGradeChange(e) {
    ...
    if (index > 0) {
      ...
      this.renderLeaderboard('grade', grade);
    }
  },

  renderLeaderboard(filter, grade = null) {
    let data = this.data.leaderboard;

    if (filter === 'grade' && grade) {
